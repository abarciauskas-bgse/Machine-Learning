...
If $M$ is the number of mistakes made by the perceptron classifier, it is equivalent to the number of times the leave one out classifier also makes a mistake. Since we can estimate the expected risk using the leave-one-out classifier and the number of iterations made by the perceptron classifier is upper-bounded by $(\frac{R}{\gamma})^{2}$ (Novikoff, 1962), we can bound the expected risk of the perceptron classifier by $\frac{M}{n}$
...
Risk is binomial and so it's variance can be lower bounded by the expected value of the squared risk.

$$Var(R_{n}^{D}(g_{n})) = \mathbb{E}\{(R_{n}^{D})^{2}\} - \mathbb{E}\{R_{n}^{D}\}^{2}$$

$$\geq \mathbb{E}\{(R_{n}^{D})^{2}\}$$

The value of the risk for which this expectation is maximized is the expected value of the risk ($R_{n}^{D}(g_{n}) \sim Bin(n,p)$) when $p=\frac{1}{2}$ and the variance can be lower bounded when the expectated value of the risk 1.

This is always greater than or equal to the probablity that just over half the data are 1's when we favor 0's.

Risk being equal to 1 when $\mathbb{P}\{Y=1\} = \mathbb{P}\{Y=0\} = \frac{1}{2}$ which happens when the data $n$ is near as possible to (favoring the half which has realized number $\frac{n-1}{2}$) $\frac{1}{2}$ with probability:

$$\mathbb{E}\{(R_{n}^{D})^{2}\} \geq \mathbb{P}\bigg\{Bin(n, \frac{1}{2}) = \frac{n+1}{2} \bigg\}$$
