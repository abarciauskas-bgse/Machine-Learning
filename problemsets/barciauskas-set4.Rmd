...
$w$ is a linear combination of all the points that are at the margin distance so w is a linear combination of these 2.
...
Data are linearly seperable by the weight vector:
...
$$\bigg\{w : -1 < w_{0} < 0 and w_{1} = ... = w_{d} = 1\bigg\}$$
...
$$\sum_{m=1}^{d}x_{m,i} = 1 \text{ $d$ is the dimension of $x$}$$
...
Where $y_{j} = -1$, $y_{i} = 1$ and $\sum_{m=1}^{d}x_{m,j} = 0$, $\sum_{m=1}^{d}x_{m,i} = 1$
...
$$w^{T}x = w_{0}^{*} + kw_{1,...,d}^{*} \text{ where $k$ is the number of non-zero elements of $x$ and $w*$ is the optimal weight vector}$$
...
$$\gamma(w) = \frac{w_{0} + \frac{d+1}{2}}{\sqrt{w_{0}^{2} + d}}$$
$$\gamma(w) = \frac{1}{2 \sqrt{\frac{d^{2}}{4} + d} }$$
...
*Is this the gaussian kernel?*

*Part 2*

Generalize to $$\mathbb{R}^{d}$$

$$=\sum_{i=1}^{\infty} \frac{1}{n!}(\sqrt{(x^{T}x)}\sqrt{y^{T}y})^{n}exp(- \frac{1}{2}(x^{T}x)- \frac{1}{2}(y^{T}y))$$
...
$$K(x,y) = \big \langle \phi(x),\phi(y) \big \rangle$$
$$= \big \langle \binom(\phi_{1}(x),\phi_{2}(x)), \binom(\phi_{1}(y),\phi_{2}(y)) \big \rangle$$
...
$$\phi(x)_{\mu} = \sum_{i=1}^{\mu} \mathbb{I}_{s_{i} substring of x}$$
